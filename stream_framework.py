...
def run_stream(params, times, df, g, model, weights, anomaly_edge_model,
    ...
    for epoch, t in enumerate(time_track):
        ...
        new_edges = df[df.t.isin(t)][['s', 'd', 'l']].to_numpy()
        ...
        if len(new_edge_indexes) > 0:
            edge_index = torch.cat((edge_index, torch.LongTensor(new_edge_indexes).T.to(device)), axis=1)
        ...
        for e, edge_emb in enumerate(edge_embs):
            ...
            if (start_test_index != -1 and index >= start_test_index) or (start_test_index == -1):
    ...
    return tp, tn, fp, fn, scores_and_labels, time.time() - total_time
