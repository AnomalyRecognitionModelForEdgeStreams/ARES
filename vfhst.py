...
__all__ = ["VeryFastHalfSpaceTrees"]


class VFHSTBranch(tree.base.Branch):
...
class VFHSTLeaf(tree.base.Leaf):
...
def make_padded_tree(limits, height, padding, rng=random, **node_params):
    ...
    return VFHSTBranch(left=left, right=right, feature=on, threshold=at, **node_params)


class VeryFastHalfSpaceTrees(anomaly.base.AnomalyDetector):
    """Very Fast Half-Space Trees (HST).
    ...
    def score_and_learn_one(self, x):
        if not self.trees:
            self.trees = [
                make_padded_tree(
                    limits={i: self.limits[i] for i in sorted(x)},
        ...
        for t in self.trees:
            for depth, node in enumerate(t.walk(x)):
                if node.r_window < self.current_window:
                    if node.r_window == self.current_window - 1:
                        node.r_mass = node.l_mass
                        node.l_mass = 0
                    else:
                        node.r_mass = 0
